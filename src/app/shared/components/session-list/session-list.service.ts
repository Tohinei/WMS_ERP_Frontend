...
export type SortColumn = keyof Session | '';
...
function sort(
  ...
  const validColumn = column as keyof Session;
...
function matches(session: Session, term: string, pipe: PipeTransform): boolean {
  const lowerTerm = term.toLowerCase();

  return (
    (session.sessionName?.toLowerCase().includes(lowerTerm) ?? false) ||
    (session.path?.toLowerCase().includes(lowerTerm) ?? false) ||
    (session.icon?.toLowerCase().includes(lowerTerm) ?? false) ||
    (session.createdAt?.toString().toLowerCase().includes(lowerTerm) ??
      false) ||
    (session.updatedAt?.toString().toLowerCase().includes(lowerTerm) ?? false)
  );
}
...
export class OrdersService {
  ...
  private _countries$ = new BehaviorSubject<Session[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    let sessionList: any[] = this.sharedDataService.getSessionList() ?? [];
    let countries = sort(sessionList, sortColumn, sortDirection);
