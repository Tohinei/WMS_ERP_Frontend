...
export type SortColumn = keyof Menu | '';
...
function sort(
  ...
  const validColumn = column as keyof Menu;
...
function matches(menu: Menu, term: string, pipe: PipeTransform): boolean {
  const lowerTerm = term.toLowerCase();

  return (
    (menu.menuName?.toLowerCase().includes(lowerTerm) ?? false) ||
    (menu.description?.toLowerCase().includes(lowerTerm) ?? false) ||
    (menu.createdAt?.toString().toLowerCase().includes(lowerTerm) ?? false) ||
    (menu.updatedAt?.toString().toLowerCase().includes(lowerTerm) ?? false)
  );
}
...
export class OrdersService {
  ...
  private _countries$ = new BehaviorSubject<Menu[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    let menuList: any[] = this.sharedDataService.getMenuList() ?? [];
    let countries = sort(menuList, sortColumn, sortDirection);
